...
public class SmoothFollowAgent : CameraController
{
    ...
    private void OnEnable()
    {
        if (target == null)
        {
            var locator = MultiCameraManager.Instance.GetPlayerObject.GetComponentInChildren<FollowTargetLocator>();
            if (locator == null)
            {
                return;
            }
    ...
    void SetPosition()
    {
        _position = Vector3.Lerp(_position, target.position + Vector3.up * height+target.right*offsetDirection*offsetDistance, Time.deltaTime * followStiffness);
